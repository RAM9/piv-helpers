...
```bash
./rsa.gen.zsh 9d john@doe.net
./secp.gen.zsh 9a john@doe.net
```
...
```
./john@doe.net/9a/9a.secp384r.pfx
./john@doe.net/9d/9d.rsa.pfx
```
...
After selecting the "Authentication" Certificate you can use the "import file" button on the right side to import the `9a` prefixed SECP ECCP384 encrypted PFX file.
...
After selecting the "Key Management" Certificate you can use the "import file" button on the right side to import the `9d` prefixed RSA2048 encrypted PFX file.
...
- Check that you're loading the correct certificate to the correct slot (ECC P384 to 9a, RSA2048 to 9d)
- Ensure your YubiKey PIV application has been initialized with a PIN

**`sc_auth identities` shows no devices:**
...
**Authentication fails after pairing:**
...
**Login works but keychain still requires password:**
...
**Multiple YubiKeys causing conflicts:**
